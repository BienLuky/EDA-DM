...
from ddim.models.diffusion import ResnetBlock, AttnBlock, nonlinearity, Downsample, Upsample
...
class QuantResBlock(BaseQuantBlock, TimestepBlock):
    ...
    def forward(self, x, emb=None, split=0):
        ...
        """
        if split != 0 and isinstance(self.skip_connection, nn.Identity) == False and self.skip_connection.split == 0:
            return checkpoint(
    ...
    def _forward(self, x, emb, split=0):
        ...
        if self.use_scale_shift_norm:
            ...
            h = self.out_layers(h)
        if split != 0:
            return self.skip_connection(x, split=self.split) + h
...
class QuantAttentionBlock(BaseQuantBlock):
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        ...
        for m in self.modules():
            if isinstance(m, QuantModule):
                m.set_quant_state(weight_quant, act_quant)
            elif isinstance(m, (QuantQKMatMul, QuantSMVMatMul)):
                m.set_quant_state(weight_quant, act_quant)
    ...
    def forward(self, x, temb=None, split=0):
        ...
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                ...
                x = self.nin_shortcut(x, split=self.split)
        out = x + h
        return out

class Quantdownsample_resnet_down(BaseQuantBlock):
    def __init__(
        self, last_layer: QuantResnetBlock, downsample: Downsample, act_quant_params: dict = {}):
        super().__init__(act_quant_params)
        self.QuantResnetBlock = last_layer
        self.downsample = downsample
        self.downsample.last_layer = None
        self.split = 0

    def forward(self, x, temb=None, split=0):
        if split != 0:
            self.split = split
        x = self.QuantResnetBlock(x, temb, self.split)
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        ...
        self.downsample.conv.set_quant_state(weight_quant, act_quant)

class Quantupsample_resnet_down(BaseQuantBlock):
    def __init__(
        self, last_layer: QuantResnetBlock, upsample: Upsample, act_quant_params: dict = {}):
        super().__init__(act_quant_params)
        self.QuantResnetBlock = last_layer
        self.upsample = upsample
        self.upsample.last_layer = None
        # self.upsample.last_resblock_layer=None
        self.split = 0
...
class Quantdownsample_attn_down(BaseQuantBlock):
    def __init__(
        self, last_layer: QuantAttnBlock, downsample: Downsample, act_quant_params: dict = {}):
        super().__init__(act_quant_params)
        self.QuantAttnBlock = last_layer
        self.downsample = downsample
        self.downsample.last_layer=None
        

    def forward(self, x):
        x = self.QuantAttnBlock(x)
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        ...
        self.use_weight_quant = weight_quant
        self.use_act_quant = act_quant
        self.QuantAttnBlock.set_quant_state(weight_quant, act_quant)
        self.downsample.conv.set_quant_state(weight_quant, act_quant)

class Quantupsample_attn_down(BaseQuantBlock):
    def __init__(
        self, last_layer: QuantAttnBlock, upsample: Upsample, act_quant_params: dict = {}):
        super().__init__(act_quant_params)
        self.QuantAttnBlock = last_layer
        self.upsample = upsample
        self.upsample.last_layer=None
